#!/bin/sh
# Renders src/public/config.js.template (copied verbatim into the built
# dist/ by Vite, see vite.config.ts) into /config.js from real container
# env vars, on every container start -- the client-side counterpart to
